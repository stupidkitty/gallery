...
{
    private $filename = 'galleries_categories.xml';
    private $urlManager;
    ...
    public function create(Sitemap $sitemap)
    {
        ...
        foreach ($models->batch(200) as $categories) {
            foreach ($categories as $category) {
                $sitemap->addItem($this->urlManager->createAbsoluteUrl(['/gallery/category/ctr', 'slug' => $category->slug]), strtotime($category->updated_at), $sitemap::DAILY, 0.7);
            }
        }
    }
